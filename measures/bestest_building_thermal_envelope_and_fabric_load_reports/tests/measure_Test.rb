...
class BESTESTBuildingThermalEnvelopeAndFabricLoadReports_Test < MiniTest::Unit::TestCase
  # class level variable
  @@co = OpenStudio::Runmanager::ConfigOptions.new(true)

  def epw_path
    # make sure we have a weather data location
    epw = OpenStudio::Path.new(File.dirname(__FILE__)) / OpenStudio::Path.new('DRYCOLDTMY.epw')
  ...
  def sql_path(test_name)
    "#{run_dir(test_name)}/ModelToIdf/EnergyPlusPreProcess-0/ExpandObjects-0/EnergyPlus-0/eplusout.sql"
  end
  ...
  def setup_test(test_name,model_in_path,idf_output_requests)
    ...
    unless File.exist?(sql_path(test_name))
      ...
      wf = OpenStudio::Runmanager::Workflow.new('modeltoidf->energypluspreprocess->expandobjects->energyplus')
      wf.add(@@co.getTools)
  ...
  def test_case_600
    test_name = 'test_case_600'
    model_in_path = "#{File.dirname(__FILE__)}/600_test_output.osm"

    # create an instance of the measure
    measure = BESTESTBuildingThermalEnvelopeAndFabricLoadReports.new
